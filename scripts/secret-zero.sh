...
# Shared HMAC token: the Gitea webhook signs its deliveries with it and the
# EventListener's github interceptor verifies them. One value, two consumers —
# so it belongs to secret zero like everything else. It used to exist only on
# the author's machine, generated by nothing, which quietly made the CI loop
# non-reproducible from git.
WEBHOOK="$DIR/webhook-secret.env"
if [[ -f "$WEBHOOK" ]]; then
  echo "[keep] webhook secret already present"
else
  echo "[gen]  webhook secret"
  echo "webhook_secret=$(openssl rand -hex 24)" > "$WEBHOOK"
  chmod 600 "$WEBHOOK"
fi
