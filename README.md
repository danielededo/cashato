...
Every screenshot below is the app running on the **synthetic dataset** in
[`demo/`](demo/README.md) — a fictional Mario Bianchi, generated by
`demo/generate.py` and parsed by the real parsers. No real statement is
involved.
...
| | |
|---|---|
| ![Transaction detail](docs/img/transaction-detail.png) | ![Wealth](docs/img/investments.png) |
| **Why this row got this category.** MCC 5411, 100% confidence, the file it came from, and the dedup key that made it survive being imported twice. | **Wealth, as the statements declare it.** Month-end balances carried forward per account. Here every contribution went somewhere the statements never itemized, and the page says so instead of inventing a portfolio. |
| ![Manage](docs/img/manage.png) | ![Dark theme](docs/img/dashboard-dark.png) |
| **Reconciliation and coverage.** Parsed movements against the balances the statements themselves print — a non-zero discrepancy localizes a lost or invented row to one account and date range. | **Light and dark**, Italian or English, from the same build. |
